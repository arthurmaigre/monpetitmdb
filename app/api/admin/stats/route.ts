...
export async function GET(req: NextRequest) {
  try {
    ...
    const [r24, r7, e24, e7, v24, v7, vTotal, vDone, vOldest] = await Promise.all([
      ...
      supabaseAdmin.from('biens').select('id', { count: 'exact', head: true }).eq('statut', 'Toujours disponible').not('derniere_verif_statut', 'is', null),
      supabaseAdmin.from('biens').select('derniere_verif_statut').eq('statut', 'Toujours disponible').not('derniere_verif_statut', 'is', null).order('derniere_verif_statut', { ascending: true }).limit(1),
    ])
