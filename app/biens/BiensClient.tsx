                  ...
                  <tbody>
                    {filtered.map(bien => (
                      <tr key={bien.id}>
                        ...
                        <td className="sticky-col" style={{ left: '40px', width: '80px', minWidth: '80px' }}>{bien.photo_url ? <img src={bien.photo_url} alt="" className="list-thumb" onMouseEnter={e => { const r = e.currentTarget.getBoundingClientRect(); let md = (bien as any).moteurimmo_data || {}; if (typeof md === 'string') try { md = JSON.parse(md) } catch {} if (typeof md === 'string') try { md = JSON.parse(md) } catch {}; const urls = md.pictureUrls?.length > 0 ? md.pictureUrls : [bien.photo_url]; setHoverPhoto({ urls, x: r.right + 16, y: r.top, idx: 0 }) }} /> : <div className="list-thumb-empty">-</div>}</td>
