...
MODE_INIT = True  # TEST : toujours en mode init
...
LOG_PATH       = BASE_DIR / "scraper_test.log"
PROGRESS_PATH  = BASE_DIR / "progress_test.json"
...
# ── LIMITES TEST ──────────────────────────────────────
TEST_MAX_PAGES_P1  = 2    # max 2 pages par combinaison en phase 1
TEST_MAX_BIENS_P2  = 15   # arrêter phase 2 après 15 biens insérés en Supabase
TEST_COMMUNES = {
...
KEYWORDS = {
    ...
    "Travaux lourds": [
        # Tier 1 — haute précision (toujours actifs)
        "à rénover",
        "gros travaux",
        "à remettre en état",
        "rénovation complète",
...
def prefiltre_travaux(texte: str, dpe: str = None) -> bool:
    ...
    Objectif : réduire les appels API inutiles sur des biens sans signal.
    """
    t = (texte or "").lower()
    signaux = [
...
def _load_few_shot_travaux(base_dir: Path, n: int = 6) -> str:
    """
    Charge les N derniers exemples du fichier ai_learning_travaux.json
    qui ont été corrigés par l'utilisateur (score_corrige != None).
    Retourne un bloc texte à injecter dans le prompt.

    Amélioration progressive :
    - Au départ : aucun exemple → Claude utilise uniquement les règles
    - Quand un utilisateur corrige un score en frontend → score_corrige est mis à jour
      dans Supabase, puis sync vers ai_learning_travaux.json au prochain run
    - Les corrections alimentent automatiquement le few-shot des runs suivants
    """
    ...
    try:
        ...
        # Prendre uniquement les exemples avec correction utilisateur
        corriges = [e for e in data.get("exemples", []) if e.get("score_corrige") is not None]
        # Prendre les N plus récents, diversifiés si possible (un par score)
        par_score = {}
...
def _save_learning_travaux(base_dir: Path, bien_id: str, description: str,
                           ...
                           commentaire: str):
    """
    Sauvegarde un exemple de scoring dans ai_learning_travaux.json.
    score_corrige = None jusqu'à correction éventuelle par l'utilisateur en frontend.
    Le frontend met à jour score_travaux dans Supabase → sync_files_after_save()
    uploade le fichier → le prochain run récupère les corrections via sync_files_on_startup().
    """
    path = base_dir / "ai_learning_travaux.json"
    try:
        if path.exists():
        ...
        # Éviter les doublons sur bien_id
...
def score_travaux_with_ai(base_dir: Path, description: str, dpe: str,
    ...
    """
    Score le niveau de travaux d'un bien via Claude API (vision + texte).
    Analyse : description + DPE + année + jusqu'à 3 photos.
...
{{"score": <1-5>, "commentaire": "<justification courte 1-2 phrases max>"}}"""
    ...
    # Construire le contenu multimodal (texte + photos)
    content = []

    # Ajouter toutes les photos disponibles
    if photo_urls:
        import base64
...
async def search_by_keyword(page, keyword, strategie, cookies_accepted,
    ...
    for page_num in range(1, TEST_MAX_PAGES_P1 + 1):
        if stop_pagination:
            break
...
async def scrape_listing_detail(page, url, strategie_active: str = "Locataire en place"):
    ...
    strategie_active : "Locataire en place" ou "Travaux lourds"
      - Locataire en place → détection is_occupied + extraction loyer/bail
      - Travaux lourds     → pré-filtre + scoring IA (description + DPE + année + photos)
    """
    data = {"url": url}
    try:
        ...
        try:
            ...
            if page_vide or any(ind in body_lower or ind in title_lower for ind in cf_indicators):
                ...
                log.warning(f"  ⛔ Ban detecte ({raison}) — pause 3min...")
                await asyncio.sleep(180)
                await page.goto(url, timeout=30000, wait_until="domcontentloaded")
                await asyncio.sleep(5)
                body_text = await page.inner_text("body")
                ...
                if page_vide or any(ind in body_lower or ind in title_text.lower() for ind in cf_indicators):
                    log.warning(f"  ⛔ Toujours banni — URL marquee echouee")
                    return "cloudflare"
        ...
        try:
            ...
            if ad_data:
                for attr in ad_data.get("attributes", []):
                    ...
                    # Accès extérieur (jardin, terrasse, balcon)
                ...
                # ── NOUVEAU : URLs de toutes les photos (pour scoring vision) ──
                images = ad_data.get("images", {})
                urls_photos = images.get("urls", []) or images.get("urls_large", [])
                if urls_photos:
                    data["_all_photo_urls"] = urls_photos  # toutes les photos
        ...
        try:
            photo_url = None
            # Priorité : photos depuis __NEXT_DATA__ (plus fiable)
            if all_photo_urls:
        ...
        if strategie_active == "Locataire en place":
            ...
            prix_fmt = f"{int(data['prix']):,}".replace(",", " ") + " €" if data.get("prix") else "prix N/A"
            dpe_fmt  = data.get("dpe", "NC")
            log.info(f"  ✓ {data.get('type_bien','?')} | {data.get('ville','?')} | {prix_fmt} | DPE {dpe_fmt} | Score travaux {score}/5")
...
async def main():
    ...
    log.info("=" * 60)
    log.info(f"SCRAPER MDB v5 TEST — {mode_label} | max {TEST_MAX_PAGES_P1} pages P1 | max {TEST_MAX_BIENS_P2} biens P2")
    ...
    # Filtrer KEYWORDS selon les stratégies choisies
    keywords_actifs = {k: v for k, v in KEYWORDS.items() if k in strategies_choisies}

    # ── Sélection métropoles (TEST : limité à TEST_COMMUNES) ──
    if METROPOLES_FILTRE:
        metropoles_choisies = [m for m in METROPOLES_FILTRE if m in TEST_COMMUNES]
    else:
        noms = list(TEST_COMMUNES.keys())
        log.info(f"\n{'─'*60}")
        log.info("SÉLECTION DES MÉTROPOLES (TEST)")
        log.info(f"{'─'*60}")
        for i, nom in enumerate(noms, 1):
            log.info(f"  {i} — {nom}")
        log.info(f"  0 — Toutes ({len(noms)})")
        log.info(f"{'─'*60}")
        choix_str = input("Choix (ex: 0  ou  1 2) : ").strip()
    ...
    communes_actives = {k: v for k, v in TEST_COMMUNES.items() if k in metropoles_choisies}
    log.info(f"Communes actives : {', '.join(communes_actives.keys())} (TEST)")
    ...
    async with async_playwright() as pw:
        ...
        for i, url in enumerate(urls_to_process, 1):
            ...
            if new_count >= TEST_MAX_BIENS_P2:
                log.info(f"  🛑 TEST : limite {TEST_MAX_BIENS_P2} biens atteinte — arrêt phase 2")
                break
            ...
            if result == "timeout":
                ...
                log.info("  🏢 Résidence gérée (tourisme/étudiants/seniors) — hors stratégie, ignoré")
                done_set.add(url)
            elif result is None:
                ...
                # Sauvegarde learning travaux (après upsert pour avoir l'ID)
