...
STRATEGIES = {
    ...
    "Division": {
        "valid": [
            r"(appartement|maison|bien|propri[eé]t[eé]|logement|surface)\s+.{0,15}divisible",
            r"divisible\s+en\s+\d+\s*(lots?|logements?|appartements?|studios?)",
            r"possibilit[eé]\s+de\s+division",
            r"division\s+possible",
            r"potentiel\s+de\s+division",
            r"diviser\s+en\s+\d+",
            r"cr[eé]er\s+(des\s+lots|\d+\s+logements|plusieurs\s+logements|deux\s+logements)",
            r"transformer\s+en\s+plusieurs\s+(lots?|logements?|appartements?)",
            r"id[eé]al\s+(pour\s+)?(la\s+)?division",
        ],
        "exclude": [
            r"non\s+divisible",
            r"issu\s+d.une\s+division",
            r"divis[eé]e?\s+en\s+deux\s+(espaces?|parties?|zones?)",
            r"chambre\s+.{0,10}divisible",
            r"(pi[eè]ce|salon|s[eé]jour|cuisine)\s+.{0,10}divisible",
            r"(jardin|cour|terrain|parcelle)\s+.{0,15}divisible",
            r"terrain\s+divisible",
            r"parcelle\s+divisible",
        ],
    },
}
